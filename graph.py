...
print("Porównanie - DFS [Y/n]")
tmp1 = input()
print("Porównanie - DEL [Y/n]")
tmp2 = input()

if tmp1.upper() == "Y":
    table = [["Krawedzie", "mSasiedztwa [ms]", "mGrafu [ms]"]]
    graph1 = AdjacencyGraph()
    graph2 = GraphMatrix()
    for i in range(100, 1501, 100):
        result1, result2 = 0, 0
        filename = "testgraphs2\\graf_" + str(i) + ".txt"
    ...
    with open("results_dfs.csv", "w", newline="") as file:
...
if tmp2.upper() == "Y":
    ...
    for i in range(100, 1501, 100):
        ...
        print(i)
    print(tabulate(table, tablefmt="fancy_grid"))
    with open("results_del.csv", "w", newline="") as file:
        writer = csv.writer(file)
        for row in table:
            writer.writerow(row)
...
for i in range(100, 1501, 100):
    ...
    print(i)
    graph1.newGraphFromFile(filename)
    print("Graph 1 loaded")
    graph2.newGraphFromFile(filename)
    print("Graph 2 loaded")
    tmp1 = graph1.DFS()
    tmp2 = graph2.DFS()
    print(f"DFS = {tmp1 == tmp2}")
    tmp1 = graph1.DEL()
    tmp2 = graph2.DEL()
    print(f"DEL = {tmp1 == tmp2}")
    print(" ")
'''''